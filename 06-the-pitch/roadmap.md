...
| Expand golden dataset from 10 rows to 150+ real recommendation cases | Contract | Reliability target is impossible to defend with the current dataset size | M |
...
- **(a)** The most over-indexed horizon is Horizon 1, which is appropriate for early validation, but the roadmap is still relatively thin on Margin-focused initiatives and medium-term defensibility experiments.
- **(b)** The single H3 bet I'd protect if the budget got cut is "Explore personalized seller optimization memory" because it is the clearest path toward a true behavioral data moat competitors cannot easily copy.
- **(c)** The one initiative I should kill today is "Monitor cross-marketplace AI ad planning tools for partner opportunities" because it is strategically vague, non-compounding, and less important than building Hepsiburada’s proprietary marketplace-native advantage.
